create table public.event_mvp_votes (
  ...
  constraint cant_vote_self check (voter_player_id != voted_player_id)
);

create policy mvp_votes_select on public.event_mvp_votes for select using (
  public.is_group_member((select group_id from public.events where id = event_id))
);

create policy mvp_votes_insert on public.event_mvp_votes for insert with check (
  voter_player_id in (select id from public.players where user_id = auth.uid())
);

-- We need to modify load_match_result to allow NULL MVP
