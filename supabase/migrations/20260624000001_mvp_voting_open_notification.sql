-- Add mvp_voting_open notification when load_match_result is called
-- with no MVP selected, signaling that MVP voting is now open
...
begin
  ...
  if p_mvp_player_id is not null and not exists (
    select 1
  ...
  loop
    ...
    if new_boost is not null then
      update public.players set current_boost = new_boost where id = target_participation.player_id;
      update public.match_participations set boost_applied = new_boost where event_id = p_event_id and player_id = target_participation.player_id;
      if target_participation.user_id is not null then
        insert into public.notifications (user_id, type, payload) values (target_participation.user_id, 'boost_applied', jsonb_build_object('event_id', p_event_id, 'player_id', target_participation.player_id, 'reason', new_boost->>'reason', 'modifiers', new_boost->'modifiers'));
      end if;
    else
      update public.players set current_boost = case when coalesce((current_boost->>'partidos_remaining')::integer, 0) <= 1 then null else jsonb_set(current_boost, '{partidos_remaining}', to_jsonb(((current_boost->>'partidos_remaining')::integer - 1))) end where id = target_participation.player_id;
      update public.match_participations set boost_applied = null where event_id = p_event_id and player_id = target_participation.player_id;
    end if;
  end loop;

  update public.players set current_boost = null where current_boost is not null and coalesce((current_boost->>'partidos_remaining')::integer, 0) <= 0;

  if p_mvp_player_id is not null then
    insert into public.notifications (user_id, type, payload) select mvp_player.user_id, 'mvp_awarded', jsonb_build_object('event_id', p_event_id, 'player_id', p_mvp_player_id) from public.players mvp_player where mvp_player.id = p_mvp_player_id and mvp_player.user_id is not null;
  end if;

  -- NEW: if no MVP was selected, notify players that MVP voting is open
  if p_mvp_player_id is null then
    insert into public.notifications (user_id, type, payload)
    select distinct player.user_id, 'mvp_voting_open'::public.notification_type,
      jsonb_build_object('event_id', p_event_id, 'group_id', target_event.group_id)
    from public.match_participations mp
    join public.players player on player.id = mp.player_id
    where mp.event_id = p_event_id
      and mp.team in ('A', 'B')
  ...
  insert into public.notifications (user_id, type, payload) select player.user_id, 'match_result_loaded', jsonb_build_object('event_id', p_event_id, 'team_a_score', p_team_a_score, 'team_b_score', p_team_b_score, 'mvp_player_id', p_mvp_player_id) from public.players player where player.group_id = target_event.group_id and player.user_id is not null and player.archived_at is null;
end;
$$;
