...
export function PlayerCardPreview({
  ...
  return (
    <article
      aria-live="polite"
      className={`relative mx-auto flex aspect-[2/3] w-full max-w-[280px] flex-col border-2 bg-concrete-overlay ${tierStyles.shadow} ${
        highlightMvp ? 'border-amber-300 shadow-[0_0_28px_rgba(251,191,36,0.5)]' : tierStyles.borderColor
      }`}
    >
      ...
      <div className="relative z-10 flex px-6 pt-6 pb-2">
        <div className="flex flex-col items-center border-r border-white/20 pr-5">
          <span className={`font-headline text-5xl font-black leading-none ${tierStyles.textColor}`}>{overall}</span>
          <span className="mt-1 font-mono text-sm font-bold text-white/70">{position}</span>
        </div>
      ...
      <div className="relative z-10 mt-3 flex flex-col items-center px-4 text-center">
        <h2 className="w-full truncate font-headline text-2xl font-black italic tracking-widest uppercase text-white">
          {name}
        </h2>
      </div>

      <div className="relative z-10 mt-auto grid grid-cols-3 gap-x-4 gap-y-3 border-t border-white/10 bg-black/40 px-6 py-5">
